package ch1;
/**
 * Lyrics.java
 * 
 * Code Description: Prints the first three verses of the song
 * 		'Scary Little Green Men' by Ozzy Osbourne.
 * 
 * @author peters
 * @version 9/13/20
 * @contact 1009734@palisadessd.net
 */

public class Lyrics 
{
  /**
   * main method - Prints the first three verses of the song
   * 	'Scary Little Green Men' by Ozzy Osbourne.
   * 
   * @param args
   */
  public static void main(String[] args)
  {
	System.out.println("'Scary Little Green Men'\n\tby Ozzy Osbourne\n");
	System.out.println("Their colors are blinding me again (I don't belong here)"
			+ "\nThe skyline is bleeding black and red (how did they get here)"
			+ "\nHow long, how long, have they been living in my head"
			+ "\nHow long, how long, we bite our tongues and play pretend\n");
	System.out.println("They want us, they need us"
			+ "\nThey might just try to eat us"
			+ "\nThey'll greet us, deceive us"
			+ "\nSay, \"Take us to your leader\"\n");
	System.out.println("Everybody wants them, until we meet them"
			+ "\nEverybody wants them (it's the end)"
			+ "\nScary little green men, do you believe in"
			+ "\nScary little green men (it's the end)");
  }
}
