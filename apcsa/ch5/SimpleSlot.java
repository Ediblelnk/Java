    ...
    public static void main(String[] args) {
        ...
        while (bRunning) {
            ...
            while (dwBetAmount == -1.0) {
                System.out.print(
                    "Input how much you would like to bet or 'end' to end: "
                );
                sInput = kb.nextLine();
                ...
                try //try to determine a bet amount
                {
                    ...
                    Exception eOne //special cases for unusual inputs
                ) {
                    if (
                        sInput.startsWith("$") //sometimes $ is before amount
                    ) {
                        try {
                            dwBetAmount = Math.abs(
                                Double.valueOf(sInput.substring(1))
                            );
                        } catch (
                            Exception eTwo //case for unknown input
                        ) {
                            System.out.println(
                        ...
                        sInput.startsWith("en") //case for end
                    ) {
                        bRunning = false;
                        dwBetAmount = 0.0;
                    }
                    //case for unknown input
            ...
            if (bDoubleWin) dStanding += (2 * dwBetAmount);
            //a double win means earning the amount bet
            else if (bTripleWin) dStanding += (20 * dwBetAmount);
            //a triple win means earning double the amount bet
            else dStanding -= dwBetAmount; //a loss means losing the amount bet
