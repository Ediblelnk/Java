...
public class IntegerList {
    ...
    public void selectionSort() {
        ...
        for (iPlace = 0; iPlace < iList.length; iPlace++) {
            ...
            for (iIndex = iPlace; iIndex < iList.length; iIndex++) {
                if (iList[iSweepMin] > iList[iIndex]) iSweepMin = iIndex;
            }
    ...
    public void replaceFirst(int iOldVal, int iNewVal) {
        ...
        if (
            iResult != -1 //if value was found in list
    ...
    public void replaceAll(int iOldVal, int iNewVal) {
        ...
        for (iIndex = 0; iIndex < iList.length; iIndex++) if (
            iList[iIndex] == iOldVal //instance of oldval
    ...
    public void sortDecreasing() {
        ...
        for (iPlace = 0; iPlace < iList.length; iPlace++) {
            ...
            for (iIndex = iPlace; iIndex < iList.length; iIndex++) {
                if (iList[iSweepMax] < iList[iIndex]) iSweepMax = iIndex;
            }
    ...
    public int binarySearchDec(int iTarget) {
        ...
        while ((!bFound) && (iMin <= iMax)) {
            iMid = (iMax + iMin) / 2;
            if (iList[iMid] == iTarget) bFound = true;
            else if (iTarget < iList[iMid]) iMin = iMid + 1;
            //bounds set up for descending order
            else iMax = iMid - 1;
        }

        if (
            bFound //returns index if target found
        ) iRet = iMid;
