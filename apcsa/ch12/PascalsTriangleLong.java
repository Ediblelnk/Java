...
public class PascalsTriangleLong {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
            "How many rows of Pascal's Triangle would you like to calculate?"
        );
        System.out.println(
            "(The tip of the triangle is considered the 0th row)"
        );
        System.out.print("Number of rows: ");
        iInput = kb.nextInt();

        if (
            iInput > 66 //maximum row value longs can handle is 66, round everything higher down to that
        ) {
            System.out.println("Sorry, maximum amount of rows is 66");
            iInput = 66; //set row input to 66
        }
     ...
     * @param
     */
    public static long[][] computeTriangle(int iSize) {
        //create 2d array to store and return the triangle
        long[][] iReturn = new long[iSize][iSize];
     ...
     * @param
     */
    public static long[] computeRow(int iRow, int iMax) {
        //declare return variable and previousrow variable
        long[] iReturn = new long[iMax];
        long[] iPrevious;
        //for loop variable
        int iIndex;
     ...
     * @param
     */
    public static int digits(long iVal) {
     ...
     * @param
     */
    public static String multiply(String sInput, long iTimes) {
     ...
     * @param
     */
    public static String inString(long[][] iTriangle) {
        ...
        int iMaxDigits =
            digits(iTriangle[iTriangle.length - 1][iTriangle.length / 2]) + 1;
        ...
        for (iRow = 0; iRow < iTriangle.length; iRow++) {
            ...
            for (iColumn = 0; iColumn < iTriangle[iRow].length; iColumn++) {
                ...
                if (iTriangle[iRow][iColumn] == 0) sResult += multiply(
                    " ",
                    iMaxDigits / 2
                );
                //uses the max digits and value in current focus to align when printed out
                    ...
                    multiply(
                        " ",
                        iMaxDigits - digits(iTriangle[iRow][iColumn])
                    ) +
                    iTriangle[iRow][iColumn];
            }
