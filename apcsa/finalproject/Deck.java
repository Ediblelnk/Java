...
public class Deck {
    ...
    public void dealCards(int cardnumber, Player[] players) {
        for (Player player : players)
            for (int i = 0; i < cardnumber; i++) player.hand.addCard(
                dealRandCard()
            );
    }
    ...
    public String with(Deck... otherdecks) {
        ...
        String returnString = "\t " + this.name + ":\t   ";
        for (Deck d : otherdecks) returnString += d.name + ":\t   ";
        returnString += "\n\t ";

        //add first layer for each deck
        returnString += "┌───────╥╖\t   ";
        for (int i = 0; i < otherdecks.length; i++) returnString +=
            "┌───────╥╖\t   ";
        returnString += "\n\t ";
