...
public class Game {
    ...
    public void initialize() {
        ...
        if (input == 1) {
            players[1] = new Player("Robert (AI)");
            vsAI = true;
        }
        ...
        for (int i = 0; i < input; i++) {
            ...
            sinput = "";
            while (sinput.length() == 0) sinput = kb.nextLine();
    ...
    public void promptTurn() {
        if (
            !vsAI //only prompts in multiplayer environment
        ) {
            System.out.print(
                "It is " +
                    (players[turnplace % players.length].name +
                        (players[turnplace %
                        (players.length - 1)].name.toUpperCase().endsWith("S")
                            ? "' turn. "
                            : "'s turn. "))
            );
    ...
    private void displayGame() {
        ...
        for (Player player : players) {
            if (!player.name.equals(focusplayer.name)) System.out.print(
                player.concealed()
            );
        }
    ...
    private void takeTurn() {
        ...
        int i;
        for (i = 1; i <= options.length; i++) System.out.println(
            i + ": " + options[i - 1] + rule(options[i - 1])
        );
        System.out.println(
            i +
                ": Draw card" +
                (drawtwostacks > 0 ? "s (" + 2 * drawtwostacks + ")" : "")
        );
        ...
        if (
            playdeck.topCard().value.equals("Q") &&
            vsAI &&
            !message.endsWith("drew a card.")
        ) {
            clear(2);
            displayGame();
    ...
    private void advanceTurn() {
        if (running) {
            if (
                !vsAI //only uses turn system if not against ai
            ) {
    ...
    private void aiTurn() {
        ...
        if (
            options.length > 0 //condition where there are playable options
        ) {
        ...
        //condition where there are no playable options (the AI only draws when it must)
        else {
            if (
                drawtwostacks == 0 //regular draw
            ) {
        ...
        try {
            TimeUnit.MILLISECONDS.sleep(500 + rand.nextInt(1501));
        ...
        //add a little time delay for dramatic effect
        try {
            TimeUnit.MILLISECONDS.sleep(250 + rand.nextInt(251));
        } catch (InterruptedException ie) {
            Thread.currentThread().interrupt();
        }
        ...
        if (
            playdeck.topCard().value.equals("Q") &&
            !message.endsWith("drew a card.")
        ) aiTurn(); //ai gets to play again if they played a queen
    }
    ...
    public void declareWinner() {
        ...
        System.out.println(
            "The WINNER of this game of CRAZY EIGHTS:" +
                "\n\n!#!#!#! " +
                winner.name +
                " !#!#!#!" +
                "\n\nCongrats!"
    ...
    private Card[] findOptions(Player focusplayer) {
        ...
        for (Card c : focusplayer.hand.hand) {
            if (
                topcard.value.equals("8") && eightinput != 0 //case for when an eight is played by the previous player
            ) {
                switch (eightinput) {
                    case 1: //previous player requested hearts
                        if (c.suit.equals("♥")) valid.add(c);
                        break;
                ...
                topcard.value.equals("2") && drawtwostacks > 0 //if the last card is a 2, only other twos may be played
            ) {
                if (c.value.equals("2")) valid.add(c);
            }
            //default case for a playable card
            else {
                //if suit or value the same, it is valid, applies to NON-EIGHTS
                if (
                    c.value.equals(topcard.value) || c.suit.equals(topcard.suit)
                ) valid.add(c);
                else if (c.value.equals("8")) valid.add(c);
            }
        }
        ...
        switch (c.value) {
            case "Q": //skip next player's turn
                ...
                if (!vsAI) message =
                    ...
                    c +
                    ", skipping " +
                    players[(turnplace + turndirection) % players.length].name +
                    (players[(turnplace + turndirection) %
                    players.length].name.endsWith("s")
                        ? "' turn."
                        : "'s turn.");
                else message =
                    focusplayer.name +
            ...
            case "8": //like a wild; prompt for a suit that the next person must play
                if (
                    vsAI && focusplayer.name.equals("Robert (AI)") //case for when Robert plays an *
                ) {
                ...
                //case for when anyone BUT Robert plays an 8
                else {
                    ...
                    if (eightinput < 1 || eightinput > 4) eightinput = 1;
            ...
            case "A": //reverses the direction of play if playing with >2 players
                if (
                    players.length != 2 //will trickle to default if there are only 2 "players"
                ) {
                    if (
                        turndirection == 1 //from forward to backwards
                    ...
                    message =
                        ...
                        " played " +
                        c +
                        ", and reversed the turn direction.";
                    break;
                }
